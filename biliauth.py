...
def bilibiliAuth() -> str:
    ...
    qrcode_size = int(
        min(qrcode_window.winfo_screenwidth(), qrcode_window.winfo_screenheight())
        * 0.45
    )
    _qrcode_image = ImageTk.PhotoImage(
        Image.open(io.BytesIO(requests.get(qr_url).content)).resize(
