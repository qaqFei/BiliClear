...
def _img_face(img: cv2.typing.MatLike):
    return not isinstance(
        face_detector.detectMultiScale(
            cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), scaleFactor=1.2, minNeighbors=1
        ),
        tuple,
    )
...
def checkUser(uid: int):
    ...
    for dynamic in dynamics:
        ...
        ims = [
            _btyes2cv2im(requests.get(i["src"]).content)
            for i in dynamic["major"]["draw"]["items"]
        ]
...
def replyIsViolations(reply: dict):
    ...
    if not isp and enable_gpt:
        try:
            isp, r = (
                gpt.gpt_porn(reply_msg) or gpt.gpt_ad(reply_msg),
                f"ChatGpt - {gpt.gpt_model} 检测到违规内容",
            )
    ...
    if (
        not isp
        and enable_check_lv2avatarat
        and reply["member"]["level_info"]["current_level"] == 2
        and "@" in reply_msg
    ):
    ...
    if (
        not isp
        and enable_check_replyimage
        and reply["member"]["level_info"]["current_level"] == 2
    ):
        try:
            images = [
                requests.get(i["img_src"], headers=headers).content
                for i in reply["content"].get("pictures", [])
            ]
