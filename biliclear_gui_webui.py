...
def render():
    while True:
        ...
        root.create_rectangle(  # background
            0, 0, w, h, fillStyle="rgb(30, 30, 30)", wait_execute=True
        )
        ...
        replyPornRate = (
            (biliclear.violationsReplyCount / biliclear.replyCount * 100)
        ...
        item_y = (
            h * 0.61
            + getListItemAnimationDy(biliclear.checkedVideos, 1, 0.5)
            * (w + h)
            / 150
            * 1.25
        )
        ...
        item_y = (
            h * 0.96
            + getListItemAnimationDy(biliclear.checkedReplies, 2, 0.5)
            * (w + h)
            / 150
            * 1.25
        )
...
def addDesignateVideo():
    ...
    if isinstance(bvid, str) and bvid.startswith("BV"):
        ...
        root.run_js_code(
            f"alert('已添加BV号: {bvid}, 请耐心等待本轮推荐视频检查完毕');"
        )
    else:
...
root.move(
    int(root.winfo_screenwidth() / 2 - (w + dw_legacy) / webdpr / 2),
    int(root.winfo_screenheight() / 2 - (h + dh_legacy) / webdpr / 2),
)

with open("./res/ChillRoundGothic_Normal.otf", "rb") as f:
    root.reg_res(f.read(), "BiliClear_UIFont")
root.run_js_code(
    f"loadFont('BiliClear_UIFont', \"{root.get_resource_path("BiliClear_UIFont")}\");"
)
...
root.run_js_code(
    "_addDesignateVideo = (e) => {if (e.ctrlKey && !e.repeat && e.key.toLowerCase() == 'i') pywebview.api.call_attr('addDesignateVideo');};"
)
...
if "--window-host" in sys.argv:
    windll.user32.SetParent(
        root.winfo_hwnd(), eval(sys.argv[sys.argv.index("--window-host") + 1])
    )
