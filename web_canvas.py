...
class WebCanvas:
    def __init__(
        ...
        self._web_init_var = {"width": width, "height": height, "x": x, "y": y}
        self._destroyed = False
        self.debug = debug
        self._regres: dict[str, bytes] = {}
        self._JavaScript_WaitToExecute_CodeArray: list[str] = []
        threading.Thread(
            target=webview.start, kwargs={"debug": self.debug}, daemon=True
        ).start()
        self._init()
        if hidden:
            self.withdraw()

    def title(self, title: typing.Union[str, None]) -> str:
        if title is None:
    ...
    def winfo_focus(self) -> bool:
        return self._web.focus

    def winfo_hwnd(self) -> int:
        return self._web_hwnd

    def winfo_legacywindowwidth(self) -> int:
        return self._web.evaluate_js("window.innerWidth;")

    def winfo_legacywindowheight(self) -> int:
    ...
    def run_js_code(
        self, code: str, threading_: bool = False, add_code_array: bool = False
    ):
        if add_code_array:
            ...
            if threading_:
                threading.Thread(
                    target=self._web.evaluate_js, args=(code,), daemon=True
                ).start()
            else:
    ...
    def run_js_wait_code(self):
        self._web.evaluate_js(
            f"JavaScript_WaitToExecute_CodeArray = {self._JavaScript_WaitToExecute_CodeArray};"
        )
        self._web.evaluate_js("process_jswteca();")
        self._JavaScript_WaitToExecute_CodeArray.clear()

    def Add_Code_To_JavaScript_WaitToExecute_CodeArray(self, code: str):
        self._JavaScript_WaitToExecute_CodeArray.append(code)

    def add_thing_to_javascript(self, name: str, value: typing.Any):
    ...
    def create_rectangle(
        self,
        x0: typing.Union[int, float],
        y0: typing.Union[int, float],
        x1: typing.Union[int, float],
        y1: typing.Union[int, float],
        fillStyle: typing.Union[str, None] = None,
        strokeStyle: typing.Union[str, None] = None,
        ...
        code = (
            self._set_style_fill_stroke(fillStyle, strokeStyle)
            + f"\
    ...
    def create_text(
        ...
        code = (
            self._set_style_fill_stroke(fillStyle, strokeStyle)
            + self._set_style_font_textAlign_textBaseline_direction(
    ...
    def clear_rectangle(
        ...
        self.run_js_code(
            f"ctx.clearRect({x0},{y0},{x1-x0},{y1-y0});", threading_, wait_execute
        )

    def clear_canvas(
        self, threading_: bool = False, wait_execute: bool = False
    ) -> None:
        self.run_js_code(
    ...
    def dereg_event(self, name: str, callback: typing.Callable) -> None:
        setattr(self._web.events, name, getattr(self._web.events, name) - callback)
    ...
    def _init(self) -> None:
        self._web.resize(
            width=self._web_init_var["width"], height=self._web_init_var["height"]
        )
        ...
        self._file_server = http.server.HTTPServer(
            ("localhost", self._web_port + 1), WebCanvas_FileServerHandler
        )
        threading.Thread(
            target=lambda: self._file_server.serve_forever(poll_interval=0), daemon=True
        ).start()
